...
async def price_per_m2():
    ...
    db = get_db()
    data = db.table("warehouses").select("price_eur, surface_m2").execute()
...
async def by_department():
    ...
    db = get_db()
    data = db.table("warehouses").select("department, price_eur, surface_m2").execute()
...
async def price_trends():
    ...
    db = get_db()
    data = db.table("warehouses").select("transaction_date, price_eur, surface_m2").execute()
...
async def top_communes():
    ...
    db = get_db()
    data = db.table("warehouses").select("commune, department, price_eur, surface_m2").execute()
